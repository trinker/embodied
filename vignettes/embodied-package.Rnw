...
\section{Functions for Longer Videos}

Larger mp4 files may cause errors or unexpected hangups due to extended processig time and large file sizes.  The user may want to consider breaking the job into smaller subcomponents and using elementary, non-wrapper functions including \texttt{gridify\_pdf}.

The following script demonstrates the process of converting a larger .mp4 video into a PDF with gridlines.


<<heavy_work_flow, eval=FALSE>>=
